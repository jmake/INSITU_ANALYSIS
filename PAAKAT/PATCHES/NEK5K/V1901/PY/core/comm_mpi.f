...
#ifdef PLEPP 
      !
      integer(4)     :: commi, gcomm 
      character(64)  :: typeij, namei, namej, string

      !
      call mpi_initialized(mpi_is_initialized, ierr)      !< 0.0
      if (.not.mpi_is_initialized) call mpi_init(ierr)    !< 0.0  
      !
      gcomm  = comm
      typeij = "PLEPP"
      namei  = "X?"
      namej  = "Y?"
      !call init_argvs(namei, namej, 1) ! 1==print
      !call init_plepp(typeij, namei, namej, 0_4, gcomm)
      call plepp_simple(gcomm, 0) !  1==print 
      !
      call mpi_comm_dup(gcomm,newcommg,ierr)              !< 0.0
      !
#else
      !
      call mpi_initialized(mpi_is_initialized, ierr)      !< 0.0
      if (.not.mpi_is_initialized) call mpi_init(ierr)    !< 0.0
      call mpi_comm_dup( comm,newcommg,ierr)              !< 0.0
      !
#endif 
      !
      newcomm = newcommg
...
!                        plepp_dim, lcomm)
      ...
      subroutine XXX_init_plepp(typeij, namei, namej, dimij, lcomm)
      !
      ! SEE :
      ...
      integer        :: lrank = -1, lsize = -1, error = -1
      !
#ifdef PLEPP 
     ...
     &                   dimij, lcomm)
      ...
      subroutine XXX_init_argvs(namei, namej, show)
      implicit none
      ...
      do iarg = 1, command_argument_count()
        ...
        !
        if(token(1:5)=="HEART") then
          namei  = "ALYA"
          namej  = "HEMELB"
        endif
        if(token(1:4)=="BODY") then
          namej  = "ALYA"
          namei  = "HEMELB"
        endif
        !
      enddo
      if(show==1) print *, "[init_argvs] --name:",  
     &         "'"//trim(namei)//"'","'"//trim(namej)//"'"
      ! 
      !key   = "??"
      !token = "--name"
      !call plepp_analyse_argvs(trim(token), len_trim(token))
      !call plepp_get_argvs(key); !print *, "[init_argvs] --name:",  "'"//trim(key)//"'"  
      !!
